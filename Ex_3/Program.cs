/* Задача №3. Пример частотного массива для текстовых данных: Входные данные:
...
string text = "Частотный анализ – это один из методов криптоанализа, основывающийся на предположении"
                + "о существовании нетривиального статистического распределения отдельных символов "
                + "и их последовательностей как в открытом тексте, так и шифрованном тексте, которое "
...
for (int i = 0; i < length; i++)
{
    ...
    persent = Math.Round((count / text.Length) * 100, 2);
    Console.WriteLine($"Символ  {text[i]} встречается  {count} раз. Процент {persent} %");
}
